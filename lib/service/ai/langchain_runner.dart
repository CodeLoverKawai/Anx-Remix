...
class CancelableLangchainRunner {
  ...
  AIChatMessageToolCall? _extractToolCallFromText(String content) {
    ...
    try {
      ...
      if (decoded is Map<String, dynamic> && decoded.containsKey('name')) {
        ...
        final parameters = decoded['parameters'] ?? decoded['arguments'] ?? decoded['params'];
        if (name.isNotEmpty && parameters is Map<String, dynamic>) {
          return AIChatMessageToolCall(
            id: 'text_call_${DateTime.now().millisecondsSinceEpoch}',
            name: name,
            argumentsRaw: jsonEncode(parameters),
  ...
  AIChatMessage _hydrateToolArguments(AIChatMessage message) {
    var toolCalls = List<AIChatMessageToolCall>.from(message.toolCalls);
    var mutated = false;

    if (toolCalls.isEmpty && message.content.isNotEmpty) {
      final extracted = _extractToolCallFromText(message.content);
      if (extracted != null) {
        toolCalls.add(extracted);
        mutated = true;
      }
    }
