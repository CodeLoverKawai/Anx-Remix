...
class AiChatStreamState extends ConsumerState<AiChatStream> {
  ...
  List<Map<String, String>> _filteredCommands = [];

  List<Map<String, String>> _getSlashCommands(BuildContext context) {
    return [
      {
        'command': '/explain',
      ...
      },
      {
        'command': '/mindmap',
        'label': 'Mindmap',
        'prompt': 'Generate a markdown mindmap of the key concepts in: ',
        'icon': 'schema',
      },
      {
        'command': '/translate',
  ...
  void _onInputChanged() {
    ...
    if (text.startsWith('/')) {
      ...
      final allCmds = _getSlashCommands(context);
      final filtered = allCmds.where((cmd) => cmd['command']!.startsWith(query)).toList();
      setState(() {
  ...
  void _selectSlashCommand(Map<String, String> cmd) {
