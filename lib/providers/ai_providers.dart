...
class AiProviders extends _$AiProviders {
  ...
  List<AiProvider> build() {
    ...
    try {
      ...
      // Ensure all default built-in providers exist in the list
      final defaultServices = buildDefaultAiServices();
      bool modified = false;
      final now = DateTime.now();

      for (final option in defaultServices) {
        final index = list.indexWhere((p) => p.id == option.identifier);
        if (index == -1) {
          ...
          // If the built-in provider already exists, make sure its logoAsset is up to date
          final existing = list[index];
          if (existing.logoAsset != option.logo) {
            list[index] = existing.copyWith(logoAsset: option.logo);
            modified = true;
          }
        }
      }
