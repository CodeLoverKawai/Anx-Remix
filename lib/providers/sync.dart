...
class Sync extends _$Sync {
  ...
  Future<void> syncData(
    ...
    try {
      ...
      await syncSettings(finalDirection);
      ...
      await syncFiles();
      await syncDictionaries(finalDirection);
  ...
  Future<void> syncDictionaries(SyncDirection direction) async {
    ...
    if (direction == SyncDirection.upload || direction == SyncDirection.both) {
      ...
      for (final dict in localDicts) {
        ...
        if (!io.File(dict.path).existsSync()) continue;

        final existsInRemote = remoteManifestList.any((r) => r['md5'] == dict.md5);
        if (!existsInRemote) {
          if (!await client.isExist('anx/dictionaries')) {
            await client.mkdirAll('anx/dictionaries');
          }
    ...
    // Download dictionaries missing locally
    if (direction == SyncDirection.download || direction == SyncDirection.both) {
      ...
      for (final remoteDict in remoteManifestList) {
        ...
        final localDictIndex = localDicts.indexWhere((l) => l.md5 == rMd5);
        final bool localHas = localDictIndex != -1;
        ...
        if (!localHas) {
          targetLocalPath = join(dictDir.path, '${DateTime.now().millisecondsSinceEpoch}_$rName.mdx');
          needDownload = true;
        } else {
          final localDict = localDicts[localDictIndex];
          existingId = localDict.id;
          if (!io.File(localDict.path).existsSync()) {
            targetLocalPath = join(dictDir.path, '${DateTime.now().millisecondsSinceEpoch}_$rName.mdx');
            localDict.path = targetLocalPath;
            await dictionaryDao.update(localDict);
            needDownload = true;
          }
        }
          ...
          Future<void>(() async {
            try {
              ...
              if (remoteDict['isActive'] == true) {
                await dictionaryDao.setActive(dictId);
              }
            } catch (e) {
              AnxLog.severe('Failed to download dictionary $rName: $e');
              if (!localHas) {
                await dictionaryDao.delete(dictId);
              }
            }
          });
        }
