...
const getCSS = ({ fontSize,
  ...
  const isLinuxWebKit = ua.includes('AppleWebKit') && !ua.includes('Chrome') && ua.includes('Linux');
  const bgCSSValue = isLinuxWebKit ? `${backgroundColor} !important` : 'transparent !important';
...
const open = async (file, cfi) => {
  console.log('book.js: open() called, file size:', file.size, 'cfi:', cfi)
  try {
    const reader = new Reader()
    globalThis.reader = reader
    console.log('book.js: calling reader.open()')
    await reader.open(file, cfi)
    console.log('book.js: reader.open() completed successfully')
    ...
    console.error('book.js: Exception in open():', e)
    throw e;
  }
}
...
console.log('book.js: Starting fetch for URL:', url)
fetch(url)
  .then(res => {
    console.log('book.js: fetch resolved, status:', res.status)
    return res.blob()
  })
  .then(blob => {
    console.log('book.js: blob created, size:', blob.size)
    return open(new File([blob], new URL(url, window.location.origin).pathname), initialCfi)
  })
  .then(() => {
    console.log('book.js: open chain completed successfully')
  })
  .catch(e => console.error('book.js: fetch/open chain error:', e))
