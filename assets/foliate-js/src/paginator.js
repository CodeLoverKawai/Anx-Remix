...
class View {
  ...
  constructor({ container, onExpand }) {
    ...
    Object.assign(this.#element.style, {
      ...
      ...(isLinuxWebKit ? {} : { contentVisibility: 'auto' }),
  ...
  expand() {
    ...
    if (this.#column) {
      ...
      const pageCount = Math.ceil(contentSize / (this.#size || 1))
      const expandedSize = pageCount * this.#size

      console.log(`[Paginator:expand:column] side=${side}, size=${this.#size}, contentSize=${contentSize}, pageCount=${pageCount}, expandedSize=${expandedSize}, rootWidth=${rootRect.width}, rootHeight=${rootRect.height}, bodyTextLength=${this.document.body.innerText?.length}`)
      ...
      console.log(`[Paginator:expand:scrolled] side=${side}, contentSize=${contentSize}, expandedSize=${expandedSize}, margin=${margin}`)
...
export class Paginator extends HTMLElement {
  ...
  async #goTo({ index, anchor, select }) {
    if (index === this.#index) await this.#display({ index, anchor, select })
      ...
      const onLoad = detail => {
        ...
        this.setStyles(this.#styles, detail.doc)
  ...
  setStyles(styles, doc = this.#view?.document) {
    this.#styles = styles
